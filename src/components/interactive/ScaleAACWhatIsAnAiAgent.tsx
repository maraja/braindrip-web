...
export default function ScaleAACWhatIsAnAiAgent() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>Understanding What Is an AI Agent? has real implications for how AI systems are built and deployed. What makes this concept especially important in practice?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>### Beyond Chat: Completing Real Work  The shift from chatbot to agent represents a qualitative change in what AI systems can accomplish. A chatbot can explain how to refactor a codebase; an agent can actually do the refactoring — reading files, making changes, running tests, and iterating until the</p>
        </div>
      )}
    </div>
  );
