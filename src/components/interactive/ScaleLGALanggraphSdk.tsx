...
export default function ScaleLGALanggraphSdk() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does LangGraph SDK matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> One client for all targets -- the same code works against local dev, self-hosted, and cloud deployments by changing only the URL and API key. Streaming made simple -- SSE parsing, reconnection, and event typing are handled internally, eliminating boilerplate.</p>
          </div>
        </div>
      )}
    </div>
