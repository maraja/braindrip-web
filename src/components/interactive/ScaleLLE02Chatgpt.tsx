...
export default function ScaleLLE02Chatgpt() {
  const [revealed, setRevealed] = useState(false);
  return (
    <div style={baseStyle}>
      <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#2C3E2D', marginBottom: 10, letterSpacing: '0.05em' }}>\u26A1 REAL-WORLD IMPACT</p>
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>Understanding ChatGPT has real implications for how AI systems are built and deployed. What makes this concept especially important in practice?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>### The Fastest-Growing Consumer Application in History  ChatGPT\'s growth metrics were staggering: 1M users in 5 days, 100M monthly active users in roughly 60 days. By January 2023, it was the dominant topic in tech media, mainstream news, social media, education policy, and corporate strategy discu</p>
        </div>
      )}
    </div>
  );
