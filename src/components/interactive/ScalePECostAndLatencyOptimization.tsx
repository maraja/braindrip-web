...
export default function ScalePECostAndLatencyOptimization() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Cost and Latency Optimization matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>The Cost-Quality Pareto Frontier:</strong> Every LLM application operates on a trade-off curve between cost and quality. Without optimization, most applications sit far from the Pareto frontier — paying more than necessary for the quality they achieve.</p>
          </div>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Latency and User Experience:</strong> Response latency directly affects user experience and engagement. Studies on conversational AI show that response times above 3 seconds reduce user satisfaction by 15-25%, and times above 5 seconds increase abandonment rates by 30-40%.</p>
          </div>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Scaling Feasibility:</strong> Some LLM applications are economically infeasible without optimization. A document processing pipeline that costs 0.50 per document using a frontier model might need to process 10 million documents — a 5 million cost.</p>
          </div>
        </div>
      )}
    </div>
