...
export default function ScaleRLAtariAndArcadeGames() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Atari and Arcade Games matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> DQN\'s significance was not in achieving the highest possible score on any single game. It was in demonstrating generality: a single algorithm and architecture, with no game-specific tuning, could learn human-level control across dozens of qualitatively different tasks from raw sensory input.</p>
          </div>
        </div>
      )}
    </div>
