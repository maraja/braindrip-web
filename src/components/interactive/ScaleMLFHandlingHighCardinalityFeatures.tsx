...
export default function ScaleMLFHandlingHighCardinalityFeatures() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Handling High-Cardinality Features matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> High-cardinality features are ubiquitous in industry: e-commerce (product IDs, seller IDs), advertising (campaign IDs, creative IDs), finance (merchant codes, account IDs), and healthcare (diagnosis codes, medication IDs). Ignoring these features discards valuable signal; naively one-hot encoding them creates intractable models.</p>
          </div>
        </div>
      )}
    </div>
