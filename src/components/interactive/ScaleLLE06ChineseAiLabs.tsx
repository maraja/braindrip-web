...
export default function ScaleLLE06ChineseAiLabs() {
  const [revealed, setRevealed] = useState(false);
  return (
    <div style={baseStyle}>
      <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#2C3E2D', marginBottom: 10, letterSpacing: '0.05em' }}>\u26A1 REAL-WORLD IMPACT</p>
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>Understanding Chinese AI Labs: The Global Competition Landscape has real implications for how AI systems are built and deployed. What makes this concept especially important in practice?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>The Chinese AI lab ecosystem matters because it transformed AI from a Western-dominated field into a genuinely global, multi-polar competition. By 2025, the monthly aggregate paper output from Chinese AI institutions exceeded that of US institutions in quantity, and the quality gap on frontier capab</p>
        </div>
      )}
    </div>
  );
