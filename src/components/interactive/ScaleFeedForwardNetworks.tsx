...
export default function ScaleFeedForwardNetworks() {
  const [revealed, setRevealed] = useState(false);
  return (
    <div style={baseStyle}>
      <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#2C3E2D', marginBottom: 10, letterSpacing: '0.05em' }}>⚡ REAL-WORLD IMPACT</p>
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>The FFN layers typically use a hidden dimension 4x the model dimension. What fraction of a Transformer's parameters live in the FFN layers?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>Roughly two-thirds (~67%) of all parameters are in FFN layers. In GPT-3's 175B parameters, over 115B are FFN weights. Research shows these layers act as "key-value memories" storing factual knowledge, which is why knowledge editing techniques target FFN layers specifically.</p>
        </div>
      )}
    </div>
  );
