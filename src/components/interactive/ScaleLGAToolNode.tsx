...
export default function ScaleLGAToolNode() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does ToolNode matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> Eliminates manual dispatch — without ToolNode, you would write boilerplate to match tool call names to functions, invoke them, and format results. Parallel execution — multiple tool calls in one LLM turn are executed concurrently, reducing latency.</p>
          </div>
        </div>
      )}
    </div>
