...
export default function ScaleMLFDbscan() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does DBSCAN matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> DBSCAN is essential when cluster shapes are unknown or irregular. It excels in spatial data analysis (geographic clustering, astronomical surveys), anomaly detection (noise points are natural outliers), and any setting where the number of clusters is not known a priori.</p>
          </div>
        </div>
      )}
    </div>
