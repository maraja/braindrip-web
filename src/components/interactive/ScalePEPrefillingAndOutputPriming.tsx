...
export default function ScalePEPrefillingAndOutputPriming() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>Understanding Prefilling and Output Priming has real implications for how AI systems are built and deployed. What makes this concept especially important in practice?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>### Format Compliance in Production  For applications where model output is parsed by downstream code (JSON APIs, data pipelines, structured reports), format compliance is not optional — a malformed response breaks the pipeline. Without prefilling, format compliance for JSON output is approximately </p>
        </div>
      )}
    </div>
  );
