...
export default function ScaleLGATokenStreaming() {
  const [revealed, setRevealed] = useState(false);
  return (
    <div style={baseStyle}>
      <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#2C3E2D', marginBottom: 10, letterSpacing: '0.05em' }}>\u26A1 REAL-WORLD IMPACT</p>
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Token Streaming matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> Perceived latency drops dramatically -- Users see the first token in under a second even if the full response takes 15 seconds. Chat UIs feel natural -- Token-by-token rendering matches the experience users expect from modern AI assistants.</p>
          </div>
        </div>
      )}
    </div>
