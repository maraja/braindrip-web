...
export default function ScaleRLMonteCarloTreeSearch() {
  const [revealed, setRevealed] = useState(false);
  return (
    <div style={baseStyle}>
      <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#2C3E2D', marginBottom: 10, letterSpacing: '0.05em' }}>\u26A1 REAL-WORLD IMPACT</p>
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>Understanding Monte Carlo Tree Search has real implications for how AI systems are built and deployed. What makes this concept especially important in practice?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>MCTS made it possible to achieve superhuman performance in Go, a game where the branching factor (~250 legal moves per position) renders traditional minimax search with alpha-beta pruning completely infeasible. Beyond games, MCTS is used in scheduling, combinatorial optimization, program synthesis, </p>
        </div>
      )}
    </div>
  );
