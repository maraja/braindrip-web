...
export default function ScaleCVCImageInterpolationAndResampling() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Image Interpolation and Resampling matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> Every image resizing operation -- from displaying a thumbnail to preparing training data at a fixed input size for a neural network -- requires interpolation. The choice of method directly affects visual quality and downstream accuracy.</p>
          </div>
        </div>
      )}
    </div>
