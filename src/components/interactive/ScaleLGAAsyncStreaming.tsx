...
export default function ScaleLGAAsyncStreaming() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Async Streaming matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> Concurrent request handling -- A single server process can handle hundreds of streaming connections without thread-per-request overhead. Framework compatibility -- FastAPI, Starlette, and other ASGI frameworks require async handlers for streaming responses.</p>
          </div>
        </div>
      )}
    </div>
