...
export default function ScaleAACModelContextProtocol() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Model Context Protocol (MCP) matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Ecosystem Composability:</strong> MCP\'s primary value is composability. A growing registry of community-built MCP servers covers databases (PostgreSQL, SQLite), developer tools (GitHub, GitLab, Linear), communication (Slack, email), file systems, web search, and more.</p>
          </div>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Separation of Concerns:</strong> MCP cleanly separates the AI reasoning layer (the model and its host) from the tool execution layer (servers). This means: tool developers do not need to understand LLM prompt engineering, AI application developers do not need to understand every API they integrate with, and tools can be updated independently of the AI application.</p>
          </div>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Standardization for Enterprise:</strong> For organizations deploying AI agents, MCP provides a standard interface for security review, access control, and audit logging. Instead of reviewing custom integration code for every tool, security teams can review the MCP protocol layer once and then evaluate individual servers against a consistent standard.</p>
          </div>
        </div>
      )}
    </div>
