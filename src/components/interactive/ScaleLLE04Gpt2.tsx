...
export default function ScaleLLE04Gpt2() {
  const [revealed, setRevealed] = useState(false);
  return (
    <div style={baseStyle}>
      <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#2C3E2D', marginBottom: 10, letterSpacing: '0.05em' }}>\u26A1 REAL-WORLD IMPACT</p>
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>Understanding GPT-2: Language Models Are Unsupervised Multitask Learners has real implications for how AI systems are built and deployed. What makes this concept especially important in practice?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>### The Scaling Hypothesis Takes Shape  GPT-2 was the first major evidence that simply making language models bigger — more parameters, more data — could produce qualitatively new capabilities. GPT-1 needed fine-tuning for each task; GPT-2 could often handle tasks zero-shot.</p>
        </div>
      )}
    </div>
  );
