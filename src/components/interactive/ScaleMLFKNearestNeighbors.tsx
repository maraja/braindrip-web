...
export default function ScaleMLFKNearestNeighbors() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does K-Nearest Neighbors matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> KNN is the simplest non-parametric classifier, requiring zero assumptions about the data distribution. It is a universal approximator: as n   and K/n  0 (with K  ), the KNN error rate converges to at most twice the Bayes optimal error rate for K=1, and to the Bayes rate itself for growing K.</p>
          </div>
        </div>
      )}
    </div>
