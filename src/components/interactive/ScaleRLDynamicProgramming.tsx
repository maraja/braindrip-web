...
export default function ScaleRLDynamicProgramming() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>How does Dynamic Programming matter in practice?</p>
      {!revealed ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ padding: '0.75rem 1rem', background: '#C76B4A0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
            <p style={{ fontSize: '0.82rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0 }}><strong>Key Impact:</strong> DP provides the correctness benchmark for all of reinforcement learning. When we say Q-learning "converges to the optimal policy," we mean it converges to the same solution DP would compute -- but without requiring the model.</p>
          </div>
        </div>
      )}
    </div>
