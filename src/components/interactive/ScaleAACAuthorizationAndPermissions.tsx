...
export default function ScaleAACAuthorizationAndPermissions() {
  ...
  return (
    <div style={baseStyle}>
      ...
      <p style={{ fontSize: '0.9rem', color: '#3D4F3E', lineHeight: 1.5, marginBottom: 12 }}>Understanding Authorization and Permissions has real implications for how AI systems are built and deployed. What makes this concept especially important in practice?</p>
      {!revealed ? (
        ...
        <div style={{ padding: '0.75rem 1rem', background: '#C76B4A' + '0C', borderRadius: 10, borderLeft: '3px solid #C76B4A' }}>
          <p style={{ fontSize: '0.9rem', color: '#2C3E2D', lineHeight: 1.5, margin: 0, fontWeight: 500 }}>### Blast Radius Containment  When an agent makes an error -- and all agents eventually will -- the damage is limited to what its permissions allow. An agent with read-only database access cannot corrupt data, no matter how badly it malfunctions.</p>
        </div>
      )}
    </div>
  );
