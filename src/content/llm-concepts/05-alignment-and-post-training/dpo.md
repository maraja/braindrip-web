...
- **SimPO (Simple Preference Optimization, 2024)**: Uses the average log-probability of a response as the implicit reward (rather than the log-ratio with a reference model), removing the need for a reference model entirely and adding a target reward margin $\gamma$ that separates winning from losing responses. SimPO achieves stronger performance than DPO on AlpacaEval 2 and Arena-Hard while being simpler to implement and more memory-efficient.
...
4. **"SimPO: Simple Preference Optimization with a Reference-Free Reward" (Meng et al., 2024)** -- Eliminates the reference model entirely by using average log-probability as the implicit reward, achieving strong results with less complexity.
5. **"Self-Play Preference Optimization for Language Model Alignment" (Wu et al., 2024)** -- Demonstrates that models can improve through self-play without external preference annotations.
