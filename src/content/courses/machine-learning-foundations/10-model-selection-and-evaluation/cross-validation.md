...
- **Hyperparameter Tuning**: CV provides the objective function that grid search, random search, and Bayesian optimization maximize.
- **Model Comparison**: Statistical tests (paired t-test, Wilcoxon) are applied to CV fold-level scores.
- **Learning Curves**: Generated by running CV at multiple training set sizes.
- **Classification Metrics / Regression Metrics**: CV produces per-fold estimates of these metrics.
- **Calibration**: Calibration should be evaluated on held-out data, ideally through cross-validation.
