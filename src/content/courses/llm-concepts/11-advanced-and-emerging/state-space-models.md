...
- **Hybrid Architectures**: The dominant production trend is now **hybrid SSM-Attention models** that combine SSM efficiency with attention's precise recall:
  ...
  - **RWKV-6/7** (RWKV Foundation, 2024-2025): An alternative linear-time architecture based on linear attention with exponential decay, supporting unlimited context and competitive quality. RWKV-6 demonstrated that pure recurrent architectures can match Transformer quality at the 7B-14B scale.
...
- **"Jamba: A Hybrid Transformer-Mamba Language Model" (Lieber et al., AI21 Labs, 2024)**: Demonstrates that hybrid architectures combining SSM and attention layers with MoE achieve the best of both worlds at production scale.
- **"Eagle and Finch: RWKV with Matrix-Valued States and Dynamic Recurrence" (Peng et al., 2024)**: The RWKV-6 paper showing that linear-time recurrent architectures can match Transformer quality while maintaining O(1) inference memory per token.
- **"xLSTM: Extended Long Short-Term Memory" (Beck et al., 2024)**: Revisits the LSTM architecture with exponential gating and matrix memory, showing competitive results with modern Transformers.
