...
**Audio Models**: Follow a similar pattern. Whisper-style audio encoders convert spectrograms into feature sequences. These are projected into the LLM's space. Models like GPT-4o process audio natively, enabling real-time voice conversation without a speech-to-text intermediate step. This "native audio" approach preserves paralinguistic features (tone, emotion, emphasis) that speech-to-text pipelines destroy.
...
The frontier has moved beyond multimodal understanding to **generation across modalities**:

- **GPT-4o** (OpenAI, 2024): Natively processes and generates text, images, and audio within a single model. Its voice mode processes speech input directly (not through transcription) and generates speech output directly (not through TTS), enabling sub-200ms conversational latency. The unified architecture avoids information loss from cascaded pipelines.
- **Gemini 2.0** (Google, 2024): Natively multimodal with text, images, audio, and video as both inputs and outputs. Includes spatial understanding for grounding language in visual scenes and real-time streaming capabilities.
- **Unified diffusion-language models**: Emerging architectures combining autoregressive language modeling with diffusion-based image generation in a single model, enabling seamless interleaving of text and image generation within a single response.

### Vision-Language-Action (VLA) Models

A significant extension connecting vision-language understanding to **physical actions** for robotics:

- **RT-2** (Google DeepMind, 2023): Outputs robot actions as text tokens, trained on both web-scale vision-language data and robotic trajectory data. Demonstrated that web knowledge transfers to physical manipulation.
- **OpenVLA** (2024): Open-source VLA model built on a standard vision-language architecture, showing the VLM-to-VLA pipeline is reproducible.

These represent the path toward embodied AI: systems that perceive visually, reason linguistically, and act physically.
...
- **"RT-2: Vision-Language-Action Models Transfer Web Knowledge to Robotic Control" (Brohan et al., Google DeepMind, 2023)**: Demonstrates that vision-language models can be extended to output robotic actions, transferring web-scale knowledge to physical manipulation tasks.
- **"GPT-4o System Card" (OpenAI, 2024)**: Documents the first production-scale any-to-any multimodal model with native audio, image, and text understanding and generation.
