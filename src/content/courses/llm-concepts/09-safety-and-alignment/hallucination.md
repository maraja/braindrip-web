...
- **FActScore** (Min et al., 2023) decomposes generated text into atomic facts and verifies each independently, providing fine-grained factuality measurement. On long-form biography generation, even the best models achieve only ~70-80% atomic fact accuracy.
- **SAFE** (Search-Augmented Factuality Evaluator, Wei et al., 2024): Google's automated factuality framework that uses an LLM agent to decompose responses into individual facts, then searches the web to verify each fact independently. SAFE achieves 72% agreement with human annotations on a per-fact basis, approaching the 76% human-human agreement rate. This enables scalable, automated hallucination measurement without human labelers.
- **Long-form factuality benchmarks** (LongFact, 2024): Evaluation suites specifically designed for measuring hallucination in long-form text generation (paragraphs to pages), which is more challenging than short-answer evaluation because errors can compound and interact.
...
- **Reasoning models reduce hallucination**: Extended chain-of-thought reasoning (o1, R1-style models) reduces factual hallucination on verifiable tasks by enabling self-verification. However, they can also produce "reasoning hallucination" -- logically coherent but factually wrong reasoning chains.
...
- Wei, J., et al. (2024). "Long-form Factuality in Large Language Models" -- Introduces the SAFE framework and LongFact benchmark for scalable, automated factuality evaluation of long-form LLM outputs using search-augmented verification.
